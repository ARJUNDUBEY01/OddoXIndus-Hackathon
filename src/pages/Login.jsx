...
import { FiMail, FiLock, FiLogIn, FiBox } from 'react-icons/fi';
...
export default function Login() {
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center p-6 ${isDark ? 'bg-slate-900' : 'bg-[#f6f6f8]'}`}>
        ...
        <main className="w-full flex items-center justify-center mt-20">
          <div className={`clay-card w-full max-w-[500px] rounded-[2rem] p-10 md:p-14 flex flex-col items-center gap-8 ${isDark ? 'border-slate-700/50' : 'border-white/40'}`}>
            ...
            <form onSubmit={handleLogin} className="w-full space-y-6">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center px-2">
                  ...
                  <Link to="/forgot-password" className="text-[#4030e8] text-xs font-bold hover:underline">
                    Forgot?
                  </Link>
                </div>
                <div className="clay-input rounded-2xl px-5 py-4 flex items-center gap-3">
              ...
              <div className="w-full pt-4">
                ...
                >
                  {loading ? (
                    ...
                    <>Sign In <FiLogIn className="text-xl" /></>
                  )}
                </button>
              </div>
            </form>
            ...
            <p className={`text-sm font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
              ...
              <Link to="/signup" className="text-[#4030e8] font-bold hover:underline">
                Create account
              </Link>
            </p>
          </div>
      ...
      {/* Decorative Orbs */}
      <div className="fixed -bottom-20 -left-20 w-80 h-80 bg-[#4030e8]/10 rounded-full blur-3xl -z-10 pointer-events-none"></div>
      <div className="fixed top-20 -right-20 w-96 h-96 bg-[#4030e8]/5 rounded-full blur-3xl -z-10 pointer-events-none"></div>
    </div>
  );
}
